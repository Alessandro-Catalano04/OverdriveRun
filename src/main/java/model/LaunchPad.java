...
/**
 * A low-profile pad that automatically launches the cube upward on contact.
 *
 * Unlike a normal jump the launch is unconditional: the player does not need
 * to press any key. The sprite is only PAD_HEIGHT pixels
 * tall so it sits flush with the ground without creating a visual step.
 */
public class LaunchPad extends AbstractEntity {
    ...
    public LaunchPad(int x, int y, int n) {
        super(x, y, GameConstants.SIZE, GameConstants.PAD_HEIGHT, n);
    }
